...
import { useApiLog } from '@/app/(examples)/text-to-speech/api-log-context';
...
export const TextToSpeechApiLog: React.FC = () => {
  const { log } = useApiLog();
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded p-2 mt-4 text-xs max-h-48 overflow-y-auto">
      <div className="font-bold mb-2 text-zinc-200">Text to Speech API Log</div>
      {log.length === 0 ? (
